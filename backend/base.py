...
def demo_populate_passenger_locations():
    ...
    return get_driver_route(passenger_source_name, passenger_dest_name)


def demo_populate_driver_locations():
    chosen = db.get_courses()[DEMO_CHOSEN_COURSE_INDEX]
    return get_driver_route(chosen["source"]["name"], chosen["destination"]["name"])
...
def handle_message(message):
    ...
    socketio.emit('response', {"driver_location": driver_location, "passenger_location": passenger_location})
